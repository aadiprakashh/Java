...
public class chess {
    ...
    public static boolean isValidMove(char[][] board, int startRow, int startCol, int targetRow, int targetCol) {
        ...
        // White pawn moves only forward
        if (board[startRow][startCol] == 'P' && startRow - targetRow != 1) {
            return false;
        }
        // Black pawn moves only forward
        if (board[startRow][startCol] == 'p' && targetRow - startRow != 1) {
            return false;
        }
